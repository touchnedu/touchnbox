...
//		console.log($(this).parents('div[class^=text-boxnumber]').attr("class"));
...
//			$('.text-boxnumber-0' + j + ' > .box-type-gen').prevAll().animate({
//				top:0, left:0
//			}, 600, 'easeOutCubic');
//			} else {
//				$('.text-boxnumber-0' + j + ' > .box-type0' + objType)
//					.offset({top:obj.attr('data-top') * 1 + $('#content-src').position().top})
//					.offset({left:obj.attr('data-left') * 1 + $('#content-src').position().left});
//				$('.text-boxnumber-0' + j + ' > .box-type0' + objType)
//												.prevAll().animate({top:0, left:0}, 600, 'easeOutCubic');
//				$('.text-boxnumber-0' + j + ' > .box-type0' + objType)
//												.nextAll().animate({top:0, left:0}, 600, 'easeOutCubic');
//			}
...
function updateReady(number) {
	var updateImgTag = "<img src='../quiz_images/" + $('#image-src').val() + 
										 ".png' data-boxNum='" + number + "'>\n"; // 이미지 주소
	
	$('div[class^=text-boxnumber]').each(function(index) {
		if(index < number) {
			var boxObj = $('#box0' + (index + 1)).attr('data-type');
			var uPositionTop = $(this).children('div[class^=box-type][data-type=' + boxObj + ']')
																.position().top - $('#content-src').position().top;
			var uPositionLeft = Math.round($(this).children('div[class^=box-type][data-type=' + boxObj + ']')
																 						.position().left - $('#content-src').position().left);
			var uWidth = $(this).children('div[class^=box-type][data-type=' + boxObj + ']').width();
			var uHeight = $(this).children('div[class^=box-type][data-type=' + boxObj + ']').height();
		
			var updateCode = "data-top='" + uPositionTop + "' " +
											 "data-left='" + uPositionLeft + "' " +
											 "data-width='" + uWidth + "' " +
											 "data-height='" + uHeight + "' " +
//											 "data-type='" + boxObj + "' " +
...
function resetAllBox() {
	...
	$boxNumber01Code = $boxNumber02Code = $boxNumber03Code 
									 = $boxNumber04Code = $boxNumber05Code = "";
};
